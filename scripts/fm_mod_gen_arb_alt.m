...
num_samples = fs/fm;  % Capture one cycle of the message signal
...
% integrated Baseband message signal
...
zero_q = ones(1,length(t))*1e-15;
bb_out = 1 * (i_signal + j*q_signal);
bb_out_i_only = 1 * (i_signal + j*zero_q);
...
if_carrier_i = amp_if_carrier_i * cos(2*pi*fc_if*t);
if_carrier_q = amp_if_carrier_q * sin(2*pi*fc_if*t);
if_zero_q = zeros(1,length(t));

if_signal_i = cos(2*pi*fc_if*t + m);
if_signal_q = sin(2*pi*fc_if*t + m);
if_signal_q_imbalanced = amp_if_carrier_q_imbalanced * sin(2*pi*fc_if*t + m + phi_offset_if_carrier_q) + dc_offset_if_q;
if_out = if_signal_i + j*if_signal_q;
if_out_i_only = if_signal_i + j*zero_q;
if_out_q_imbalanced = if_signal_i + j*if_signal_q_imbalanced;
...
rf_q_imbalanced = if_signal_q_imbalanced .* carrier_rf_q;
rf_signal = rf_i - rf_q;
rf_q_carrier_imbalanced = if_signal_q .* carrier_rf_q_imbalanced;
rf_signal_q_carrier_imbalanced = rf_i - rf_q_carrier_imbalanced;
rf_signal_if_q_imbalanced = rf_i - rf_q_imbalanced;
rf_signal_ideal = cos(2*pi*(fc_if + fc_rf)*t + m);
...
demod_if_i_mixed = rf_signal .* carrier_rf_i;
demod_if_q_mixed = rf_signal .* -carrier_rf_q;
...
[b, a] = butter(5, fc_rf/fs, 'low'); % Design a 5th order Butterworth low-pass filter
...
% Recover the original I and Q signals from the RF signal.
% This involves another mixing process with the same carrier, followed by low-pass filtering.
demod_i_mixed_bb = sqrt(2) * recovered_if_i_signal .* if_carrier_i;
demod_q_mixed_bb = sqrt(2) * recovered_if_i_signal .* -if_carrier_q;

% === 5. Low-Pass Filtering ===
% Low-pass filters are necessary to remove the high-frequency components that result from the mixing process.
[b, a] = butter(5, fc_if/fs, 'low'); % Design a 5th order Butterworth low-pass filter
...
save -v6 fm_mod_1kHz_tone_5kHz_deviation_baseband_i_only.mat bb_out_i_only;
...
save -v6 fm_mod_1kHz_tone_5kHz_deviation_100kHz_if_i_only.mat if_out_i_only;
...
fft_if_recovered = fftshift(fft(recovered_if_i_signal))/N;
fft_if_imbalanced = fftshift(fft(if_signal_q_imbalanced))/N;
plot(f, 20*log10(abs(fft_if)), 'b',f, 20*log10(abs(fft_if_recovered)), 'g');
title('IF Signal Spectrum');
xlabel('Frequency (Hz)');
ylabel('Magnitude');
xlim([fc_if-10*fm, fc_if+10*fm])
ylim([-100, 0]);

% Spectrum of RF signal
subplot(4,1,3);
fft_rf = fftshift(fft(rf_signal))/N;
fft_rf_imbalanced = fftshift(fft(rf_signal_if_q_imbalanced))/N;
fft_rf_ideal = fftshift(fft(rf_signal_ideal))/N;
plot(f, 20*log10(abs(fft_rf)), 'b', f, 20*log10(abs(fft_rf_imbalanced)), 'r',f, 20*log10(abs(fft_rf_ideal)), 'g');
title('RF Signal Spectrum');
xlabel('Frequency (Hz)');
ylabel('Magnitude');
xlim([fc_rf + fc_if - 10*fm, fc_rf + fc_if + 10*fm])
ylim([-100, 0]);

% Spectrum of demodulated signals
subplot(4,1,4);
fft_recovered_i = fftshift(fft(recovered_i_signal))/N;
fft_recovered_q = fftshift(fft(recovered_q_signal))/N;
plot(f, 20*log10(abs(fft_recovered_i)), 'b', f, 20*log10(abs(fft_recovered_q)), 'r');
